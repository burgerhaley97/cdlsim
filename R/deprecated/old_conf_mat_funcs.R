...
#' @import readxl
#' @export
get_mat_data_test <- function(state_abbreviation, file_path = "inst/extdata/extracted_files") {
  ...
  process_state_files <- function(state_abbrev) {
    ...
    patterns <- list(
      ...
      sprintf("NASS_CDL_%s1[89]_accuracy\\.xlsx$", state_abbrev),
      sprintf("NASS_CDL_%s2[0-3]_accuracy\\.xlsx$", state_abbrev)
    )
...
#' @import dplyr
#' @export
# change the row names/column names to just the first row and firt column
#
get_trans_mat <- function(df_list, categories = 1:256) {
